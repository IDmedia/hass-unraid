...
class GraphQLClient:
    def __init__(self, endpoint_url: str, api_key: str, verify_ssl: bool = True, timeout: int = 30):
    ...
    async def query(self, query: str, variables: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if r.status_code >= 400:
            # Log the server's response for debugging (often includes {"errors":[...,"message":...]}
            try:
                # if GraphQLClient doesn't have logger, skip
                self.logger
            except Exception:
                pass
            # Raise with body for context
